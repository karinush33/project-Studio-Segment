...
def complete_yourself__get_hf_client() -> InferenceClient:
    hf_token = "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"
...
def complete_yourself__ask_llama_for_name_desc(cluster_summary: str) -> tuple[str, str]:
    ...
    messages = [
        {
            "role": "user",
            "content": f"Analyze this cluster summary and provide a short name and a one-sentence description.\nSummary: {cluster_summary}\nFormat: Name: [Name], Description: [Description]"
        }
    ]
    ...
    try:
        name = output.split("Name:")[1].split("Description:")[0].strip()
        desc = output.split("Description:")[1].strip()
        return name, desc
    except:
        return "General Group", output[:50]
